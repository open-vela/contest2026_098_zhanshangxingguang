...
- **团队分工**:队长 **张焱** —— 独立完成全部项目开发(BK7258 板级适配、驱动、端侧算法、文档与演示)。
...
- **离线命令词识别(自研 MFCC + DTW)** —— 麦克风 → MFCC → DTW 模板匹配 + 拒识,**纯 C、零第三方库、Apache-2.0**,断网可用(`kws enroll/run`);并支持大赛指定唤醒词"你好,openvela",配**离线语音应答**(`kws wake`:说对→喇叭答"你好,我在";没听清→"没听清,请再说一遍")。
...
nsh> kws enroll chaxinlv # 登记"查心率"(可多录几遍)
nsh> kws enroll yundong  # 登记"开始运动"
nsh> kws enroll nihaoopenvela  # 登记指定唤醒词"你好openvela"
nsh> kws run             # 说命令词 → MATCH/REJECTED + 距离
nsh> kws list            # 查看已登记模板
nsh> kws wake            # 说唤醒词 → 语音应答"你好,我在";没听清 → "请再说一遍"
...
- **离线命令词**:**说话人相关**的模板匹配(需本人登记)、词表规模小;含糊发音会**安全拒识(不误报)**。**对近场收音较敏感**:板载麦无自动增益,贴麦说话会削波(样本顶到满量程)导致特征失真、识别率骤降,实测**嘴离麦约 8~12cm、音量正常偏大、末字吐清楚**最稳(串口会打印 `level: peak/rms` 便于自查,peak 落在约 2000~15000 为宜);enroll 与 run 的距离/念法需保持一致。**语音应答为预置离线问候**(说对→"你好,我在";没听清→"请再说一遍"),非自由对话。后续可升级到 TFLM + DS-CNN(int8)做说话人无关识别、加软件 AGC/归一降低对距离的敏感;自由对话可选联网走云端 LLM。
...
- **音频/麦克风**:硬件为单颗语音麦(MIC1)+ MIC2 接喇叭做**硬件 AEC 回采**(芯片具备边放边听/打断能力);**官方确认默认不支持双麦降噪与声源定位**。本作品语音用**单路 MIC1、16 kHz 采集**,**未实现/未验证** AEC 降噪、双麦降噪与声源定位(早期声源定位为实验代码,已取消)。
