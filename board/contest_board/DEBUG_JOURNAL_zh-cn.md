...
7. **`init_sequence(false)` 与 `lcd_display_on()` 必须成对出现**：传 `false` 的调用方
   必须在画面准备好后自行调用 `lcd_display_on()`；单独跑某条命令时不要依赖前一条命令
   遗留的显示状态。已两次踩此坑：早期 `lcdtest_one`、以及 `lcdtest_pat_draw`。
...
## 31. M3c 修订：上板实测后的两处大改（敲击→摇一摇、红灯 P40 复用修复）

> §30 是"敲击(CLICK)→开心"的初版设计。上板后发现两个真问题，本节记录 pivot 与最终实现。
> 相关代码：`src/bk7258_camera.c`（velapet 手势 + LED）、`src/bk7258_accel.c`、`src/bk7258_gc9d01.c`（新增 `lcdtest led`）。

### 31.1 踩坑：硬件 CLICK 敲击在"松板/轻壳"上不可行 → 改"摇一摇"
- 现象：`lcdtest accel g` 里怎么敲都**只出 PICKUP、零 TAP**。看数值(x/y 冲到 ±600~1910mg、z 在 +370~-1179 乱跳)——**每次"敲"都把这块轻板整体晃动了**。
- 根因：LIS2DH/SC7A20 的 CLICK 引擎要求"板子基本不动、只有尖锐冲击且在 TIME_LIMIT 内落回阈值"。松板一拍就晃，冲击拖太长 → 引擎判"非敲击"丢弃。把阈值降低(0x0c)反而更糟(慢晃也压过低阈值、超时)。TIME_LIMIT 拉满(0x7f)也没救。
- 🟢 **决策**：放弃硬件 CLICK，改用**可靠的姿态检测**做"摇一摇"：
  - 单次拿起(flat→lifted)且当前 SLEEP → `WAKE`(惊喜)；
  - **短时间内连续两次拿起**(第 2 次距第 1 次 ≤ `VP_SHAKE_GAP=6` 个采样周期)→ 判为"摇一摇" → `HAPPY`(开心)。
  - 用 `accel_tick` 计采样周期数、`last_pickup_tick` 记上次拿起，去掉了 tap/tap_guard/putdown 那套。
- 结论：**开心表情本来 camera 那路(脸凑近且居中)也能触发**，所以放弃 CLICK 不丢功能；摇一摇对松板子稳得多。

### 31.2 踩坑：情绪红灯 P40 上电处于 alt function，一直点不亮
- 现象：HAPPY 时代码同时点绿(P41)红(P40)，**绿亮红不亮**；两者用的是同一套 `gpio_set_output_high`。
- 定位：加 `lcdtest led`(直接点灯命令)对照——它能点亮红绿两灯，差别在于它**多设了一步"SYS func-select nibble → GPIO"**。P41 上电默认就是 GPIO 功能，**P40 默认在 alt function(2RX/1BCLK/…)**，只清 AON 的 SECOND_FUNC 位不够，必须把 SYS 的功能选择位也设成 GPIO。
- 试过"启动时一次性设 P40/P41 的 SYS func"，仍不稳(红只瞬亮/不亮)。
- 🟢 **最终修法**：新增 `vp_led_set(pin,on)`，**每次点灯都完整重选 GPIO 功能 + 驱动电平**(和验证过的 `lcdtest_led_set` 完全一致)，情绪灯宏 `VP_LED_RED/GREEN` 改为调它。红灯稳定点亮。
- 教训：某些脚上电默认复用功能，配 GPIO 输出必须**同时**清 AON `SECOND_FUNC` 位**和**设 SYS 功能选择位；对这类脚，"每次操作都重配"比"一次性初始化"更稳。

### 31.3 最终行为（上板实测通过）
- 平放无人 → 困(半睑慢眨)，双灯灭；
- 拿起 → 惊喜(瞳孔放大) + 绿灯；
- 摇一摇 / 脸凑近居中 → 开心(眯眼笑) + 红灯(~1s 脉冲) + 绿灯；
- 回跟随 → 红灭绿留；离开 → 回困、双灯灭。
...
## 33. 电池电量：内部 SARADC 读 VBAT + 低电红灯慢闪

> 方框图标注"4.2V VBAT 内部检测电量(ADC0)"——电池电压由 BK7258 内部 SARADC 通道0 测量，无外部分压脚。
> 相关代码：新建 `src/bk7258_battery.c`/`.h`；`src/bk7258_gc9d01.c`(`lcdtest bat`)；`src/bk7258_camera.c`(velapet 低电慢闪)。

### 33.1 SARADC 单次读(手抄 ARMino)
- openvela 固件无任何 ADC 驱动,寄存器级手抄(同 audio/accel 路子)。SARADC 基址 `0x45890000`;VBAT=通道0,内部 1/5 分压(无引脚)。
- 时钟/偏置:`sadc_cken`(SYS `0x44010030` bit5,普通 MMIO)+ GADC 模拟偏置(ana_reg2 `0x44010108`:nobuf_en[15]、sp_nt_ctrl[31:25]、refbuf/inbuf/cmp_ictrl)走 analog-SPI;分压 1/5 = ana_reg5 `adc_div`[11:10]=1,配前后夹 ana_reg9 `spi_latch1v`[9]。
- 单次:软复位 → ctrl 设 单步模式/通道0/adc_div=12(26M/2/13≈1MHz)→ 使能 → 轮询 mode 自动清零 → 读 `0x20` adc_data。稳定(raw≈6533,抖动~0.2%)。

### 33.2 标定(一点法)
- 无 eFuse 校准码,故用一点标定:万用表实测满电 **4.2V ↔ raw 6533**,`K = 4200/6533 ≈ 0.643 mV/LSB`,`mV = raw*K`。
- 百分比:锂电 4200mV=100% / 3300mV=0% 线性映射(够做"饿了"指示,不追求精确曲线)。
- `lcdtest bat` → `raw / mV / %`;满电显示 4200mV/100%,充电升、耗电降,单调跟随。
- 注:量电压是**并联**(表笔搭 BAT+/GND),不用断电;只有量电流才需断开串入。

### 33.3 低电红灯慢闪(接入 velapet,不动状态机)
- velapet 开机读一次 + 每 `VP_BAT_CHECK_EVERY`(600 帧)重读一次 `bk7258_battery_read_mv()`,`< VP_BAT_LOW_MV(3500)` 置 `low_batt`,充电恢复自动清除。
- 低电且非 HAPPY 态:红灯每 `VP_BAT_BLINK`(25 帧≈1.2Hz)翻转 = 慢闪("饿了/该充电");HAPPY 态跳过(HAPPY 自己管红灯常亮),退出后恢复慢闪;恢复电量则强制熄红。
- SARADC 读用 GADC 模拟,和音频 DAC 都走 analog-SPI,但**低频、不同帧、顺序执行**,实测互不干扰;每次读~30ms,对眼睛/跟随无感。
- 验证:临时把阈值抬到 4300(>当前 4200)强制判低电 → 慢闪、HAPPY 优先、拿起/音效/跟随全正常 → 再改回 3500 定稿。

### 33.4 结果
- 🟢 <span style="color:#1a7f37">**电量可读(mV/%)+ 低电红灯慢闪接入情绪引擎("电量=饿了")。**</span>
- 后续可选:电量百分比上屏、低电进一步"犯困"行为、充电中/满电状态(读 5V_DET/FULL_DET)。
