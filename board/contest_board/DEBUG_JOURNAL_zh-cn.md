...
### 24.3 像素格式是 UYVY,不是 YUYV

- 🔴 <span style="color:#d1242f">**ARMino 的注释是错的**</span>
  GC2145 寄存器 `0x84 = 0x02` 输出的是 **UYVY**,
  而 ARMino `dvp_gc2145.c` 把这个值标注成 "yuyv"。
  抓 PSRAM 内存 dump 确认,实际字节序是 `U Y0 V Y1`。

  为什么这个错能活很久:搞反了**不会出乱码**,而是出一张"看起来合理但颜色发飘"的图。
  最后暴露它的现象是——采样偏移挪一个字节,颜色通道就互换了。

  代码里部分标识符还叫 YUYV,是因为它对应移植来源的 ARMino 寄存器表;
  真正的转换路径 `uyvy_to_rgb565_scaled()` 用的是正确顺序。
...
- 🔴 <span style="color:#d1242f">**78ms 不是算术开销,是 PSRAM 访问模式**</span>
  `uyvy_to_rgb565_scaled()` 每个输出像素要读 3 个源字节,
  160×160 输出 ≈ **76800 次散读**。
  PSRAM 挂在 QSPI 后面,单次访问开销占主导,只有顺序突发才接近标称带宽。

**故意不优化。** 全屏预览是 bring-up 和演示路径,不是产品路径。
产品路径是:采集到 PSRAM → 在那儿做人脸检测 → 输出方向 → 只重绘眼睛。
60×60 虹膜区 7200 字节,1000 KB/s 下约 7ms,动画绝对够顺。
...
- 🟡 <span style="color:#d4a017">**这个数是下一步的输入**</span>
...
- 🟡 <span style="color:#d4a017">**下一步**</span> 人脸检测 + 方向输出。开工前需先调研:
  1. PSRAM 散读性能约束(已知 7.7 万次散读 = 78ms,算法访存模式是选型关键)
  2. AP 侧算力/内存预算(CPU 120MHz、bss 43KB、PSRAM 余约 14MB)
